...
export default function CreateLeagueModal({ onCreated, onClose }) {
  const [form, setForm] = useState({ name: '', teamName: '', type: 'redraft', scoring: 'standard', size: 12, season: '2025-26' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!form.name.trim() || !form.teamName.trim()) { setError('League name and team name are required.'); return; }
    setCreating(true);
    try {
      const { league } = await leagueApi.createLeague({ name: form.name, teamName: form.teamName, type: form.type, scoringPreset: form.scoring, leagueSize: form.size, season: form.season });
      onCreated(league);
      onClose();
      ...
      <div className="auth-modal" style={{ maxWidth: 460, width: '90vw' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="auth-input-group">
            ...
            <input className="auth-input" type="text" placeholder="My Championship League" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
          </div>
          <div className="auth-input-group">
            <label>Your Team Name</label>
            <input className="auth-input" type="text" placeholder="Team Name" value={form.teamName} onChange={e => setForm(f => ({ ...f, teamName: e.target.value }))} />
          </div>
